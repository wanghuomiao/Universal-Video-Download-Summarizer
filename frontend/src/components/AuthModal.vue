<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-[100] flex items-center justify-center" @click.self="$emit('close')">
      ...
      <div class="relative rounded-2xl w-full max-w-md mx-4 overflow-hidden animate-modal-in surface-card-strong">
        <!-- Header -->
        <div class="px-8 pt-8 pb-4">
          <div class="flex items-center justify-between mb-1">
            ...
            <button @click="$emit('close')" class="p-1 rounded-lg hover:bg-[rgba(244,235,219,0.62)] transition-colors cursor-pointer">
        ...
        <form @submit.prevent="handleSubmit" class="px-8 pb-8">
          <div class="space-y-4">
            <div>
              ...
              <input
                ...
                class="w-full h-11 px-4 rounded-xl text-sm text-text-primary placeholder:text-text-muted lux-input-field"
                :disabled="submitting"
              />
            </div>
            <div>
              ...
              <div class="relative">
                <input
                  ...
                  class="w-full h-11 px-4 pr-11 rounded-xl text-sm text-text-primary placeholder:text-text-muted lux-input-field"
                  :disabled="submitting"
                />
          ...
          <button
            type="submit"
            :disabled="submitting"
            class="w-full h-11 mt-6 rounded-full text-sm font-semibold disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-2 lux-button-primary"
          >
